...
from keras.layers import LSTM, Bidirectional, Dense, Dropout, Layer
...
class DenormalizedMAE(MeanAbsoluteError):
    ...
    def update_state(self, y_true, y_pred, sample_weight=None):
        return super().update_state(
            tf.scalar_mul(self.std, y_true),
            tf.scalar_mul(self.std, y_pred),
            sample_weight,
        )
...
class WalmartModel:
    ...
    def train(
        ...
        self.model.compile(
            optimizer,
            "mse",
            metrics=[MeanAbsoluteError(name="mae"), DenormalizedMAE(train.target_std)],
        )
        callbacks = [
    ...
    def predict(self, test: WalmartDataset, batch_size: int = 100) -> np.ndarray:
        ...
        target_mean, target_std = (
            self.means[WalmartDataset.TARGET_INDEX],
            self.stds[WalmartDataset.TARGET_INDEX],
        )
        return (
            self.model.predict(test_tf, verbose=0).flatten() * target_std + target_mean
        )
