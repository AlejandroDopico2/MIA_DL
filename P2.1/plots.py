...
def plot_errors(
    ...
    for data in datas:
        ...
        for model in models:
            preds = np.split(model.predict(data), len(data.stores))
            golds = np.split(
                data.to_numpy(model.seq_len, norm=False)[1], len(data.stores)
            )
            errors.append(
                np.mean(
                    np.stack([gold - pred for pred, gold in zip(preds, golds)]), axis=0
                )
            )
    ...
    fig.update_layout(
        ...
        template="seaborn",
        margin=dict(t=40, b=10, l=10, r=10),
        yaxis=dict(title_text="difference"),
    )
    return fig
