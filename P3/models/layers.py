...
class ResidualConvBlock(Layer):
    def __init__(
        ...
        **kwargs
        ...
        self.conv2 = Conv2D(
            n_filters, kernel_size=3, strides=1, padding="same", use_bias=bias
        )
...
class ResidualDeconvBlock(Layer):
    def __init__(
        ...
        **kwargs
        ...
        self.bn2 = BatchNormalization(momentum=momentum) if batch_norm else Identity()
        self.skip_conv = Conv2DTranspose(
            n_filters,
            kernel_size=kernel_size,
            strides=strides,
            dilation_rate=dilation,
...
def vae(
    img_size: Tuple[int, int, int], hidden_size: int, act: str, residual: bool, **args
):
    ...
    transform = Sequential([Dense(np.prod(shape)), Reshape(shape)], name="transform")(
        decoder_input
    )
...
def skip_vae(
    ...
    deconv3 = Sequential([Concatenate(-1), deconv(32, 3, **args)], name="deconv3")(
        [deconv4, conv3]
    )
    deconv2 = Sequential([Concatenate(-1), deconv(32, 3, **args)], name="deconv2")(
        [deconv3, conv2]
    )
    deconv1 = Sequential([Concatenate(-1), deconv(32, 3, **args)], name="deconv1")(
        [deconv2, conv1]
    )
    decoder_output = Conv2D(3, 1, activation=act, name="output")(deconv1)
    return (
...
class VariationalAutoEncoder(Model):
    def __init__(
        ...
        self.transform = Sequential(
            [Dense(np.prod(shape)), Reshape(shape)], name="transform"
        )
        self.deconv4 = deconv(32, 3, **args, name="deconv5")
        if skips:
            self.deconv3 = Sequential(
                [Concatenate(-1), deconv(32, 3, **args)], name="deconv3"
            )
            self.deconv2 = Sequential(
                [Concatenate(-1), deconv(32, 3, **args)], name="deconv2"
            )
            self.deconv1 = Sequential(
                [Concatenate(-1), deconv(32, 3, **args)], name="deconv1"
            )
        else:
        ...
        def kl_loss(y_true: tf.Tensor, y_pred: tf.Tensor) -> tf.Tensor:
            kl_loss = -0.5 * K.sum(
                1 + self.__mean - K.square(self.__mean) - K.exp(self.__logvar), axis=1
            )
            return kl_loss
