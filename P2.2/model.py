...
class AmazonReviewsModel:
    def __init__(
        ...
        if bidirectional:
            add_layer = lambda *args, **kwargs: Bidirectional(recurrent_layer(*args, **kwargs))
        else:
    ...
    def train(
        ...
        callbacks = [
            EarlyStopping(monitor="val_loss", patience=dev_patience, mode="min", verbose=0),
            EarlyStopping(monitor="loss", patience=train_patience, mode="min", verbose=0),
            ModelCheckpoint(path, monitor="val_loss", mode="min", save_best_only=True, save_weights_only=True, verbose=0),
        ]
        self.model.compile(optimizer, loss='binary_crossentropy', metrics=['accuracy'])
        history = self.model.fit(
            data.X_train, data.y_train, epochs=epochs, batch_size=batch_size, callbacks=callbacks,
            validation_data=(data.X_val, data.y_val)
        )
        self.model.load_weights(path)
        return history, plot_history(history.history, metrics=['loss', 'accuracy'], name=self.model.name)
