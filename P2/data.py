...
class StoreInstance:
    ...
    def split(self, pval: float = 0.0, ptest: float = 0.0) -> Tuple[StoreInstance, StoreInstance, StoreInstance]:
    ...
    def to_tf(
        ...
        inputs, targets = self.data[:-delay], self.data[delay:, WalmartDataset.TARGET_INDEX]
        if norm and params is not None:
            means, stds = params
            inputs = (inputs - means)/stds
            targets = (targets - means[WalmartDataset.TARGET_INDEX])/stds[WalmartDataset.TARGET_INDEX]
        data = timeseries_dataset_from_array(
            inputs, targets, seq_len, sampling_rate=1, start_index=0, batch_size=batch_size,
            shuffle=shuffle)
        return data 
...
class WalmartDataset:
    ...
    def split(self, pval: float = 0.0, ptest: float = 0.0) -> Tuple[WalmartDataset, WalmartDataset, WalmartDataset]:
        ...
        return WalmartDataset(train, self.dates[:-(ntest+nval)]), \
            WalmartDataset(val, self.dates[-(ntest+nval):-ntest]), WalmartDataset(test, self.dates[-ntest:])
    ...
    def to_tf(
        ...
        for store in self.stores[1:]:
            data = data.concatenate(store.to_tf(seq_len, batch_size, norm, params, shuffle))
        return data
