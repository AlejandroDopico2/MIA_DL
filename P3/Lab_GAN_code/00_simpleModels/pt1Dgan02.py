...
# Define a more complex neural network for the generator
class Generator(nn.Module):
    def __init__(self):
        super(Generator, self).__init__()
        # Input noise dimension could be 1 or more
        self.fc_mu = nn.Linear(1, 1)  # Output the mean
        self.fc_sigma = nn.Linear(1, 1)  # Output the log of standard deviation for stability
        self.fc_mu.weight.data.fill_(0.0)
        self.fc_mu.bias.data.fill_(initial_guess_mean)
...
for i in range(iterations):
    ...
    mu, sigma = generator(noise)
    gen_samples = mu + sigma * torch.randn_like(sigma)
